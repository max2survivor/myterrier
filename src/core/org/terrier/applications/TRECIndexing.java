 ...
 * <b>Properties:</b>
 * <ul>
 * <li><tt>trec.indexer.class</tt> - name of the class to use as the indexer.
 * This only applies to the Index method.</li>
 * <li><tt>trec.collection.class</tt> - name of the class to use as the
 * Collection.</li>
 * </ul>
 * 
...
public class TRECIndexing {
   ...
   * 
   * @param _path
   *          Absolute path to where the index should be created
   * @param _prefix
   *          Prefix of the index files, usually "data"
   */
  public TRECIndexing(String _path, String _prefix) {
    path = _path;
    prefix = _prefix;
    // load the appropriate collection
    final String collectionName = ApplicationSetup.getProperty(
        "trec.collection.class", "TRECCollection");
    collectionTREC = CollectionFactory.loadCollection(collectionName);
    if (collectionTREC == null) {
      logger.fatal("Collection class named " + collectionName
          + " not found, aborting");
    }
    
    // load the appropriate indexer
    String indexerName = ApplicationSetup.getProperty("trec.indexer.class",
        ApplicationSetup.BLOCK_INDEXING ? "BlockIndexer" : "BasicIndexer");
    if (indexerName.indexOf('.') == -1) indexerName = "org.terrier.indexing."
        + indexerName;
    else if (indexerName.startsWith("uk.ac.gla.terrier")) indexerName = indexerName
        .replaceAll("uk.ac.gla.terrier", "org.terrier");
    try {
      indexer = (Indexer) Class.forName(indexerName)
          .getConstructor(String.class, String.class).newInstance(path, prefix);
    } catch (ClassNotFoundException e) {
      logger.fatal("Indexer class named " + indexerName + " not found", e);
    } catch (InstantiationException ie) {
      logger.fatal("Error while instantiating Indexer class named "
          + indexerName + " : " + ie.getCause(), ie);
    } catch (Exception e) {
  ...
  /**
   * A default constructor that initialised the data structures to use for
   * indexing.
   */
  public TRECIndexing() {
  ...
  /**
   * Calls the method index(Collection[]) of the class Indexer in order to build
   * the data structures for a set of collections. This particular method of the
   * Indexer uses a set of builders for a subset of the collection and builds
   * separate data structures, which are later merged.
   */
  public void index() {
    if (Index.existsIndex(path, prefix)) {
      logger.fatal("Cannot index while an index exists at " + path + ","
          + prefix);
      return;
    }
  ...
  public void createInvertedFile() {
    if (Index.existsIndex(path, prefix)) {
      ...
      if (i == null) {} else if (i.hasIndexStructure("inverted")) {
        logger
            .fatal("Cannot create an inverted structure while an index with a inverted structure exists at "
                + path + "," + prefix);
        return;
      } else if (!i.hasIndexStructure("direct")) {
        logger
            .fatal("Cannot create an inverted structure without a direct structure in the index at "
                + path + "," + prefix);
        return;
      }
    } else {
      logger.fatal("Cannot create an inverted structure without an index at "
          + path + "," + prefix);
      return;
    }
  ...
  /**
   * Builds the direct file and lexicon. This method goes through the input
   * files specified in the <tt>collections.spec</tt> file and processes them in
   * groups of n documents, where n is specified by the property
   * <tt>bundle.size</tt>. Then, it merges the temporary lexicon files. If it
   * necessary, it calls for the optimisation of the identifiers assigned to
   * terms.
   */
  public void createDirectFile() {
    if (Index.existsIndex(path, prefix)) {
      Index i = Index.createIndex();
      if (i == null) {} else if (i.hasIndexStructure("direct")) {
        logger
            .fatal("Cannot create a direct structure while an index with a direct structure exists at "
                + path + "," + prefix);
        return;
      }
    }
    ...
    if (logger.isInfoEnabled()) logger.info("Direct index built in "
        + ((endTime - startTime) / 1000.0D) + " seconds.");
    try {
      collectionTREC.close();
    } catch (Exception e) {
      logger.warn("problem closing collection", e);
  ...
  public void createSinglePass() {
    ...
    if (ApplicationSetup.BLOCK_INDEXING) _indexer = new BlockSinglePassIndexer(
        ApplicationSetup.TERRIER_INDEX_PATH,
        ApplicationSetup.TERRIER_INDEX_PREFIX);
    else _indexer = new BasicSinglePassIndexer(
        ApplicationSetup.TERRIER_INDEX_PATH,
        ApplicationSetup.TERRIER_INDEX_PREFIX);
  ...
  public static void main(String[] args) throws IOException {
    ...
    t.index();
    long endTime = System.currentTimeMillis();
    if (logger.isInfoEnabled()) logger.info("Elapsed time="
