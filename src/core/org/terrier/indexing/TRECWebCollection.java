...
/**
 * Version of TRECCollection which can parse standard form DOCHDR tags in TREC
 * Web corpoa. A standard format DOCHDR tag from WT2G is shown below.
 * 
 * <pre>
 * &lt;DOCHDR&gt;
 ...
 * </pre>
 * 
 * TRECWebCollection parses each HTTP header as Document property. In addition,
 * the URL, IP address, date and length are parsed from the DOCHDR tags. In
 * particular, the following Document properies are set, depending on the format
 * of the DOCHDR tag:
 * <ul>
 * <li>url (all corpora)</li>
 * <li>ip (WT2G, WT10G only)</li>
 * <li>docbytelength (WT2G, WT10G, Blog06, Blogs08 only)</li>
 * <li>contenttype (WT2G, WT10G only, but usually identified in the HTTP
 * headers)</li>
 ...
 * TREC web corpora, in particular the first line of the tag. The following
 * corpora are supported.
 * <ul>
 ...
 * </ul>
 * For indexing the more recent TREC ClueWeb09 corpus, see
 * {@link WARC018Collection}.
 * 
...
public class TRECWebCollection extends TRECCollection {
  ...
  final static SimpleDateFormat dateWT2G = new SimpleDateFormat(
      "yyyyMMddHHmmss");
  static {
    dateWT2G.setTimeZone(TimeZone.getTimeZone("UTC"));
  }
  
  // final static SimpleDateFormat dateBlogs06 = new
  // SimpleDateFormat("yyyyMddHHmm");
  ...
  protected void afterPropertyTags() throws IOException {
    StringBuilder hdr = super.getTag(DOCHDR_START_LENGTH, DOCHDR_START,
        DOCHDR_END);
    if (hdr == null) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (!first) { // first line is a special case
        ...
        if ((Colon = lines[i].indexOf(':')) > 1) {
          /*
           * Content-Type: text/html becomes content-type => text/html
          ...
          DocProperties.put(lines[i].substring(0, Colon).trim().toLowerCase()
              .replaceAll("-", ""), lines[i].substring(Colon + 2).trim());
