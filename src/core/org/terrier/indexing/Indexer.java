 ...
 * <B>Properties:</b>
 * <ul>
 * <li><tt>termpipelines</tt> - the sequence of TermPipeline stages (e.g. <a
 * href="../terms/Stopwords.html">Stopwords</a> removal and <a
 * href="../terms/PorterStemmer.html">PorterStemmer</a>).
 * <li><tt>termpipelines.skip</tt> - a list of tokens which should not be
 * skipped from the term pipeline. If not set or empty, then none will be
 * skipped.</li>
 * <li><tt>indexing.max.tokens</tt> - The maximum number of tokens the indexer
 * will attempt to index in a document. If 0, then all tokens will be indexed
 * (default).</li>
 * <li><tt>ignore.empty.documents</tt> - Assign empty documents documnent Ids.
 * Default true</li>
 * <li><tt>indexing.max.docs.per.builder</tt> - Maximum number of documents in
 * an index before a new index is created, and merged later.
 * <li><tt>indexing.builder.boundary.docnos</tt> - Docnos of documents that
 * force the index being created to be completed, and a new index to be
 * commenced. An alternative to <tt>indexing.max.docs.per.builder</tt>
 * </ul>
 * 
...
public abstract class Indexer {
  ...
  /**
   * The number of documents indexed with a set of builders. If a collection
   * consists of more documents, then we need to create new builders and later
   * merge the data structures. The corresponding property is
   * <tt>indexing.max.docs.per.builder</tt> and the default value is
   * <tt>18000000</tt> (18 million documents). If the property is set equal to
   * zero, then there is no limit.
   */
  protected int MAX_DOCS_PER_BUILDER = 0;
  
  /**
   * The maximum number of tokens in a document. If it is set to zero, then
   * there is no limit in the number of tokens indexed for a document. Set by
   * property <tt>indexing.max.tokens</tt>.
   */
  ...
  /**
   * Indicates whether field information should be saved in the created data
   * structures.
   */
  ...
  /**
   * Indicates whether an entry for empty documents is stored in the document
   * index, or empty documents should be ignored.
   */
  ...
  protected String basicDirectIndexPostingIteratorClass = BasicIterablePosting.class
      .getName();
  protected String fieldDirectIndexPostingIteratorClass = FieldIterablePosting.class
      .getName();
  ...
  /**
   * Creates an instance of the class. The generated data structures will be
   * saved in the given path. The of the data is given by the prefix parameter.
   * 
   * @param _path
   *          String the path where the generated data structures will be saved.
   * @param _prefix
  ...
  /**
   * This method must be called by anything which directly extends Indexer. See:
  ...
  /**
   * An abstract method for creating the direct index, the document index and
   * the lexicon for the given collections.
   * 
   * @param collections
  ...
  /**
   * An abstract method for creating the inverted index, given that the the
   * direct index, the document index and the lexicon have already been created.
   */
  ...
  protected TObjectIntHashMap<String> fieldNames = new TObjectIntHashMap<String>(
      0);
  ...
  protected MetaIndexBuilder createMetaIndexBuilder() {
    ...
    final String[] reverseMetaKeys = ApplicationSetup.getProperty(
        "indexer.meta.reverse.keys", "docno").split("\\s*,\\s*");
    return new CompressingMetaIndexBuilder(currentIndex, forwardMetaKeys,
        metaKeyLengths, reverseMetaKeys);
  }
  ...
  protected void load_indexer_properties() {
    ...
    MAX_TOKENS_IN_DOCUMENT = Integer.parseInt(ApplicationSetup.getProperty(
        "indexing.max.tokens", "0"));
    MAX_DOCS_PER_BUILDER = Integer.parseInt(ApplicationSetup.getProperty(
        "indexing.max.docs.per.builder", "18000000"));
  }
  ...
  protected void load_field_ids() {
    ...
    if (!FieldScore.USE_FIELD_INFORMATION || FieldScore.FIELDS_COUNT == 0) return;
  ...
  /**
   * Creates the term pipeline, as specified by the property
   * <tt>termpipelines</tt> in the properties file. The default value of the
   * property <tt>termpipelines</tt> is <tt>Stopwords,PorterStemmer</tt>. This
   * means that we first remove stopwords and then apply Porter's stemming
   * algorithm.
   */
  ...
  protected void load_pipeline() {
    ...
    for (int i = pipes.length - 1; i >= 0; i--) {
      try {
        ...
        if (className.indexOf(".") < 0) className = PIPELINE_NAMESPACE
            + className;
        else if (className.startsWith("uk.ac.gla.terrier")) className = className
            .replaceAll("uk.ac.gla.terrier", "org.terrier");
        Class pipeClass = Class.forName(className, false, this.getClass()
            .getClassLoader());
        ...
        logger.warn("TermPipeline object " + PIPELINE_NAMESPACE + pipes[i]
            + " not found: " + e);
        e.printStackTrace();
      }
    }
    ...
    if ((skipTerms = ApplicationSetup.getProperty("termpipelines.skip", null)) != null
        && skipTerms.trim().length() > 0) pipeline_first = new SkipTermPipeline(
        next, last);
    else pipeline_first = next;
  }
  
  /**
   * Loads the builder boundary documents from the property
   * <tt>indexing.builder.boundary.docnos</tt>, comma delimited.
   */
  protected void load_builder_boundary_documents() {
    final String[] docnos = ApplicationSetup.getProperty(
        "indexing.builder.boundary.docnos", "").split("\\s*,\\s*");
    for (int i = 0; i < docnos.length; i++) {
      docnos[i] = docnos[i].trim();
      if (docnos[i].length() > 0) BUILDER_BOUNDARY_DOCUMENTS.add(docnos[i]);
    }
    if (BUILDER_BOUNDARY_DOCUMENTS.size() > 0) logger.info("Watching for "
        + BUILDER_BOUNDARY_DOCUMENTS.size()
        + " documents that force index builder boundaries.");
  }
  
  /**
   * Creates the data structures for a set of collections. It creates a set of
   * data structures for every <tt>indexing.max.docs.per.builder</tt>, if the
   * value of this property is greater than zero, and then it mertges the
  ...
  /**
   * Merge a series of numbered indices in the same path/prefix area. New merged
   * index will be stored at mpath/mprefix_highest+1.
   * 
   * @param mpath
   *          Path of all indices
   * @param mprefix
   *          Common prefix of all indices
   * @param lowest
   *          lowest subfix of prefix
   * @param highest
   ...
   * 
   * @param index1
   *          Path/Prefix of source index 1
   * @param index2
   *          Path/Prefix of source index 2
   * @param outputIndex
   *          Path/Prefix of destination index
   */
  protected static void mergeTwoIndices(String[] index1, String[] index2,
      String[] outputIndex) {
    ...
    if (ApplicationSetup.BLOCK_INDEXING) sMerger = new BlockStructureMerger(
        src1, src2, dst);
    else sMerger = new StructureMerger(src1, src2, dst);
   ...
   * 
   * @param mpath
   *          Common path of all indices
   * @param mprefix
   *          Prefix of target index
   * @param counterMerged
   *          - number of indices to merge
   */
  public static void merge(String mpath, String mprefix,
      LinkedList<String[]> llist, int counterMerged) {
    while (llist.size() > 1) {
      LinkedList<String[]> tmpList = new LinkedList<String[]>();
      // merge every two indices stored in the linked list
      for (int i = 0; i < llist.size(); i++) {
        String[] filename1 = llist.get(i++);
        // if the first index is the end of the linked list (which means the
        // size of the linked list
        // is odd), merge with the previous merged index.
        String[] filename2 = (i == llist.size()) ? (tmpList.removeLast())
            : llist.get(i);
        String[] outputFilename = new String[] {mpath,
            mprefix + "_" + (counterMerged++)};
        // logger.info("Merging "+ filename1 + " and " + filename2 + " to " +
        // outputFilename);
    ...
    try {
      IndexUtil.renameIndex(mpath, mprefix + "_" + (counterMerged - 1), mpath,
          mprefix);
    } catch (IOException ioe) {
  ...
  public static void main(String args[]) throws Exception {
    if (args[0].equals("--merge") && args.length == 3) {
      merge(ApplicationSetup.TERRIER_INDEX_PATH,
          ApplicationSetup.TERRIER_INDEX_PREFIX, Integer.parseInt(args[1]),
          Integer.parseInt(args[2]));
      return;
    }
    logger
        .error("Usage: org.terrier.indexing.Indexer --merge [lowid] [highid]");
  }
