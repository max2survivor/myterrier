...
public class CompressingMetaIndexBuilder extends MetaIndexBuilder implements
    Flushable {
  protected static final Logger logger = Logger
      .getLogger(CompressingMetaIndexBuilder.class);
  protected static final int MAX_MB_IN_MEM_RETRIEVAL = Integer
      .parseInt(ApplicationSetup.getProperty(
          "metaindex.compressed.max.data.in-mem.mb", "400"));
  protected static final int MAX_INDEX_MB_IN_MEM_RETRIEVAL = Integer
      .parseInt(ApplicationSetup.getProperty(
          "metaindex.compressed.max.index.in-mem.mb", "100"));
   ...
   * @param _forwardKeys
   */
  public CompressingMetaIndexBuilder(Index _index, String[] _keyNames,
  ...
  public CompressingMetaIndexBuilder(Index _index, String _structureName,
      String[] _keyNames, int[] _valueLens, String[] _forwardKeys) {
    this.index = _index;
    this.structureName = _structureName;
    this.keyNames = _keyNames;
    this.valueLensChars = _valueLens;
    if (this.keyNames.length != this.valueLensChars.length) throw new IllegalArgumentException(
    ...
    try {
      this.dataOutput = new DataOutputStream(Files.writeFileStream(_index
          .getPath()
          + "/"
          + _index.getPrefix()
          + "."
          + structureName
          + ".zdata"));
      this.indexOutput = new DataOutputStream(Files.writeFileStream(_index
          .getPath() + "/" + _index.getPrefix() + "." + structureName + ".idx"));
    ...
    if (_forwardKeys.length == 1 && _forwardKeys[0].length() == 0) _forwardKeys = new String[0];
    
    this.forwardKeyNames = _forwardKeys;
    this.forwardKeys = new int[_forwardKeys.length];
    int i = 0;
    for (String fwdKey : _forwardKeys) {
      if (!key2Index.contains(fwdKey)) throw new IllegalArgumentException(
          "Reverse key "
              + fwdKey
    ...
    for (i = 0; i < forwardKeys.length; i++) {
      forwardWriters[i] = new MultiFSOMapWriter(_index.getPath() + "/"
          + _index.getPrefix() + "." + structureName + "-" + i
          + FSOrderedMapFile.USUAL_EXTENSION,
          REVERSE_KEY_LOOKUP_WRITING_BUFFER_SIZE,
          keyFactories[i] = new FixedSizeTextFactory(
              valueLensChars[forwardKeys[i]]),
    ...
    for (i = 0; i < keyNames.length; i++) {
      this.valueLensBytes[i] = FixedSizeTextFactory
          .getMaximumTextLength(this.valueLensChars[i]);
  ...
  public void writeDocumentEntry(String[] data) throws IOException {
    ...
    for (String value : data) {
      if (value == null) value = "";
      else if (value.length() > valueLensChars[i]) throw new IllegalArgumentException(
          "Data for key "
              + keyNames[i]
              + " exceeds max string length of "
              + valueLensChars[i]
              + "(byte length of "
              + valueLensBytes[i]
      ...
      if (numberOfBytesToWrite > valueLensBytes[i]) throw new IllegalArgumentException(
          "Data for key "
              + keyNames[i]
              + " exceeds max byte length of "
              + valueLensBytes[i]
              + "(string length of "
              + valueLensChars[i]
              + "). Crop in the Document, or increase indexer.meta.forward.keylens");
      baos.write(b);
      if (numberOfBytesToWrite < valueLensBytes[i]) baos.write(spaces, 0,
          valueLensBytes[i] - numberOfBytesToWrite);
      i++;
    }
    zip.reset();
    ...
    for (i = 0; i < forwardKeys.length; i++) {
      ...
      if (lastValues[i] != null
          && data[forwardKeys[i]].compareTo(lastValues[i]) < 1) forwardKeyValuesSorted[i] = false;
  ...
  public void close() throws IOException {
    ...
    index.addIndexStructure(structureName,
        ...
        "org.terrier.structures.Index,java.lang.String", "index,structureName");
    index.addIndexStructureInputStream(structureName,
        "org.terrier.structures.CompressingMetaIndex$InputStream",
        "org.terrier.structures.Index,java.lang.String", "index,structureName");
    index.setIndexProperty("index." + structureName + ".entries", ""
        + entryCount);
    index.setIndexProperty("index." + structureName + ".compression-level", ""
        + ZIP_COMPRESSION_LEVEL);
    index.setIndexProperty("index." + structureName + ".key-names",
        ArrayUtils.join(keyNames, ","));
    index.setIndexProperty("index." + structureName + ".value-lengths",
        ArrayUtils.join(valueLensChars, ","));
    index.setIndexProperty("index." + structureName + ".entry-length", ""
        + entryLengthBytes);
    index.setIndexProperty("index." + structureName + ".data-source",
        currentOffset > MAX_MB_IN_MEM_RETRIEVAL * (long) 1024 * 1024 ? "file"
            : "fileinmem");
    index
        .setIndexProperty("index." + structureName + ".index-source",
            currentIndexOffset > MAX_INDEX_MB_IN_MEM_RETRIEVAL * (long) 1024
                * 1024 ? "file" : "fileinmem");
    // TODO emit warnings
    index.flush();
   ...
   * @throws Exception
   */
  public static void reverseAsMapReduceJob(Index index, String structureName,
      String[] keys) throws Exception {
    final HadoopPlugin.JobFactory jf = HadoopPlugin
        .getJobFactory("TerrierIndexingMeta");
    if (jf == null) throw new Exception(
  ...
  // @SuppressWarnings("deprecation")
  public static void reverseAsMapReduceJob(Index index, String structureName,
      String[] keys, HadoopPlugin.JobFactory jf) throws Exception {
    ...
    conf.set("CompressingMetaIndexBuilder.reverse.keys",
        ArrayUtils.join(keys, ","));
    ...
    // only update the index from the controlling process, so that we dont have
    // locking/concurrency issues
    index.setIndexProperty("index." + structureName + ".reverse-key-names",
        ArrayUtils.join(keys, ","));
    index.flush();
    logger.info("Time Taken = " + ((System.currentTimeMillis() - time) / 1000)
        + " seconds");
  }
   ...
   */
  // @SuppressWarnings("deprecation")
  public static class KeyedPartitioner implements
      Partitioner<KeyValueTuple,IntWritable> {
    ...
    public void configure(JobConf jc) {
      ...
      String[] keys = jc.get("CompressingMetaIndexBuilder.reverse.keys", "")
          .split("\\s*,\\s*");
      int i = 0;
      for (String k : keys) {
  ...
  static class KeyValueTuple implements WritableComparable<KeyValueTuple> {
    ...
    public boolean equals(Object other) {
      if (!(other instanceof KeyValueTuple)) return false;
  ...
  static class MapperReducer
    ...
    protected void configureMap() throws IOException {
      reverseKeyCount = jc.getInt(
          "CompressingMetaIndexBuilder.reverse.keyCount", 0);
      reverseKeyNames = jc.get("CompressingMetaIndexBuilder.reverse.keys", "")
          .split("\\s*,\\s*");
      final TObjectIntHashMap<String> key2forwardOffset = new TObjectIntHashMap<String>(
          reverseKeyCount);
      final String[] forwardKeyNames = jc.get(
          "CompressingMetaIndexBuilder.forward.keys", "").split("\\s*,\\s*");
      int i = 0;
    ...
    public void map(IntWritable docid, Wrapper<String[]> _metadata,
      ...
      for (int i = 0; i < reverseKeyCount; i++) {
        collector.collect(new KeyValueTuple(reverseKeyNames[i], metadata[i]),
            docid);
      }
      reporter.progress();
    }
    ...
    protected void configureReduce() throws IOException {
      ...
      String structureName = jc.get(
          "MetaIndexInputStreamRecordReader.structureName", "");
      reverseKeyCount = jc.getInt(
          "CompressingMetaIndexBuilder.reverse.keyCount", 0);
      reverseKeyNames = jc.get("CompressingMetaIndexBuilder.reverse.keys", "")
          .split("\\s*,\\s*");
      ...
      final String[] allKeys = index.getIndexProperty(
          "index." + structureName + ".key-names", "").split("\\s*,\\s*");
      final String[] allValueLens = index.getIndexProperty(
          "index." + structureName + ".value-lengths", "").split("\\s*,\\s*");
      i = 0;
    ...
    /**
     * Reduce function. Input Key: (meta Key name, meta Key value) Value: list
     * of matching docids.
     */
    @Override
    public void reduce(KeyValueTuple metaTuple, Iterator<IntWritable> docids,
      ...
      if (currentReducingKey == null
          || !metaTuple.getKeyName().equals(currentReducingKey)) {
        if (currentReducingKey != null) {
          logger.info("currentKey was " + currentReducingKey + " ("
              + currentKeyTupleCount + " entries) new Key is "
              + metaTuple.getKeyName() + " : force closed");
          currentReducingOutput.close();
          if (duplicateKeyCount > 0) {
            logger.warn("MetaIndex key " + currentReducingKey + " had "
                + duplicateKeyCount
      ...
      reporter.progress();
      if (extraCount > 0) {
        // logger.warn("Key "+currentReducingKey + " value "+
        // metaTuple.getValue() + " had "+ extraCount
        // +" extra documents. First document selected.");
        duplicateKeyCount++;
      }
    ...
    protected void closeReduce() throws IOException {
      if (currentKeyTupleCount > 0) {
        logger.info("Finished reducing for " + currentReducingKey + ", with "
            + currentKeyTupleCount + " entries");
      }
      if (duplicateKeyCount > 0) {
        logger.warn("MetaIndex key " + currentReducingKey + " had "
            + duplicateKeyCount
      ...
      if (currentReducingOutput != null) currentReducingOutput.close();
    }
    
    /*
     * open a MapFileWriter for the specified key. This will automatically
     * promoted to the index folder when the job is finished
     */
    protected MapFileWriter openMapFileWriter(String keyName)
        throws IOException {
      ...
      logger.info("Opening MapFileWriter for key " + keyName + " - index "
          + metaKeyIndex);
      return FSOrderedMapFile.mapFileWrite(reduceTaskFileDestinations
          .toString() /* index.getPath() */
          + "/"
          + index.getPrefix()
          + "."
      ...
      //
      /*
       * return new MultiFSOMapWriter( reduceTaskFileDestinations.toString() +
       * "/" + index.getPrefix() + "." +
       * jc.get("MetaIndexInputStreamRecordReader.structureName") +
       * "-"+metaKeyIndex+FSOrderedMapFile.USUAL_EXTENSION,
       * REVERSE_KEY_LOOKUP_WRITING_BUFFER_SIZE, keyFactory = new
       * FixedSizeTextFactory(valueLength), new FixedSizeIntWritableFactory() );
       */
    }
  }
