...
/**
 * The PostingListManager is reponsible for opening the appropriate posting
 * lists {@link IterablePosting} given the MatchingQueryTerms object. Moreover,
 * it knows how each Posting should be scored.
 * <p>
 * Plugins are also supported by PostingListManager. Each plugin class should
 * implement the PostingListManagerPlugin interface, and be named explicitly in
 * the <tt>matching.postinglist.manager.plugins</tt> property.
 * <p>
 * <b>Properties:</b>
 * <ul>
 * <li> <tt>ignore.low.idf.terms</tt> - should terms with low IDF (i.e. very
 * frequent) be ignored? Defaults to true, i.e. ignored</li>
 * <li> <tt>matching.postinglist.manager.plugins</tt> - Comma delimited list of
 * PostingListManagerPlugin classes to load.</li>
 * </ul>
 * <p>
 * <b>Example Usage</b>
 * </p>
 * Following code shows how term-at-a-time matching may occur using the
 * PostingListManager:
 * 
 * <pre>
 * MatchingQueryTerms mqt;
 * Index index;
 * PostingListManager plm = new PostingListManager(index,
 *     index.getCollectionStatistics(), mqt);
 * plm.prepare(false);
...
public class PostingListManager implements Closeable {
  protected static final Logger logger = Logger
      .getLogger(PostingListManager.class);
  /**
   * A property that enables to ignore the terms with a low IDF. Controlled by
   * <tt>ignore.low.idf.terms</tt> property, defualts to true.
   */
  protected static boolean IGNORE_LOW_IDF_TERMS = Boolean
      .parseBoolean(ApplicationSetup.getProperty("ignore.low.idf.terms",
          "false"));
  
  /**
   * Interface for plugins to further alter the posting lists managed by the
   * PostingListManager
   */
  public static interface PostingListManagerPlugin {
    /**
     * process the query, given matchign query terms, index and posting list
     * manager
     */
    void processQuery(MatchingQueryTerms mqt, Index index,
  ...
  static {
    ...
    for (String pluginClass : pluginNames) {
      if (!pluginClass.contains(".")) pluginClass = "org.terrier.matching."
          + pluginClass;
      try {
        PostingListManagerPlugin p = Class.forName(pluginClass)
            .asSubclass(PostingListManagerPlugin.class).newInstance();
        _plugins.add(p);
      } catch (Exception e) {
        logger.warn("Problem loading PostingListManagerPlugin called "
            + pluginClass, e);
      }
    }
  ...
  /**
   * Create a posting list manager for the given index and statistics, and
   * populated using the specified MatchingQueryTerms.
   * 
   * @param _index
   *          - index to obtain postings from
   * @param _cs
   *          - collection statistics to obtain
   * @param mqt
   *          - MatchingQueryTerms object calculated for the query
   */
  public PostingListManager(Index _index, CollectionStatistics _cs,
    ...
    for (String queryTerm : mqt.getTerms()) {
      if (queryTerm.contains("|")) {
        ...
        addSingleTermAlternatives(alternatives, queryTerm,
            mqt.getTermWeight(queryTerm), mqt.getStatistics(queryTerm),
            mqt.getTermWeightingModels(queryTerm));
      } else {
        addSingleTerm(queryTerm, mqt.getTermWeight(queryTerm),
            mqt.getStatistics(queryTerm), mqt.getTermWeightingModels(queryTerm));
      }
    }
    for (PostingListManagerPlugin p : plugins) {
      p.processQuery(mqt, index, this);
    }
    logger.info("Query " + mqt.getQueryId() + " with " + mqt.getTerms().length
        + " terms has " + termPostings.size() + " posting lists");
    assert termPostings.size() == termStatistics.size();
  }
  
  /**
   * Add a single term to those to be matched for this query. Those with more
   * occurrences than the number of documents will be ignored if
   * IGNORE_LOW_IDF_TERMS is enabled.
   * 
   * @param queryTerm
   *          String form of the query term
   * @param weight
   *          influence of this query term in scoring
   * @param entryStats
   *          statistics to be used for this query term. If null, these will be
   *          obtained from the local Lexicon
   * @param wmodels
   *          weighting models to be applied for this query term
   * @throws IOException
   */
  public void addSingleTerm(String queryTerm, double weight,
      EntryStatistics entryStats, WeightingModel[] wmodels) throws IOException {
    LexiconEntry t = lexicon.getLexiconEntry(queryTerm);
    if (t == null) {
      logger.debug("Term Not Found: " + queryTerm);
      // previousTerm = false;
    } else if (IGNORE_LOW_IDF_TERMS
        && collectionStatistics.getNumberOfDocuments() < t.getFrequency()) {
      logger.warn("query term " + queryTerm
          + " has low idf - ignored from scoring.");
      // previousTerm = false;
    } else if (wmodels.length == 0) {
      logger.warn("No weighting models for term " + queryTerm
          + ", skipping scoring");
      // previousTerm = false;
    } else {
      termStrings.add(queryTerm);
      termPostings.add(invertedIndex.getPostings((BitIndexPointer) t));
      if (entryStats == null) entryStats = t;
      if (logger.isDebugEnabled()) logger.debug("Term " + queryTerm + " stats"
          + entryStats.toString());
  ...
  /**
   * Adds a synonym group to the matching process. EntryStatistics for all terms
   * in the group will be combined using mergeStatistics()
   * 
   * @param terms
   *          String of the terms in the synonym group
   * @param weight
   *          influence of this synonym group during retrieval
   * @param entryStats
   ...
   * @throws IOException
   */
  public void addSingleTermAlternatives(String[] terms, String stringForm,
      double weight, EntryStatistics[] entryStats, WeightingModel[] wmodels)
      throws IOException {
   ...
   *          influence of this synonym group during retrieval
   * @param entryStats
   *          statistics of the whole synonym group. If null, these will be
   *          obtained from the local Lexicon for all terms in the group will be
   *          combined using mergeStatistics()
   * @param wmodels
   *          WeightingModels for the synonym group (NOT one per member).
   * @throws IOException
   */
  public void addSingleTermAlternatives(String[] terms, String stringForm,
    ...
    for (String alternative : terms) {
      ...
      if (t == null) {
        ...
        // previousTerm = false;
          ...
          && collectionStatistics.getNumberOfDocuments() < t.getFrequency()) {
        logger.warn("query term " + alternative
            + " has low idf - ignored from scoring.");
        // previousTerm = false;
      } else if (wmodels.length == 0) {
        logger.warn("No weighting models for term " + alternative
            + ", skipping scoring");
        // previousTerm = false;
      } else {
        _le.add(t);
    ...
    if (entryStats == null) entryStats = mergeStatistics(_le
        .toArray(new LexiconEntry[_le.size()]));
    if (logger.isDebugEnabled()) logger.debug("Dijunctive term "
        + Arrays.toString(terms) + " stats" + entryStats.toString());
    termStrings.add(stringForm);
    termStatistics.add(entryStats);
    // System.err.println(entryStats.toString());
    IterablePosting[] joinedPostings = _joinedPostings
        .toArray(new IterablePosting[_joinedPostings.size()]);
  ...
  /**
   * Counts the number of terms active. If firstMove is true, it will move each
   * posting to the first posting.
   * 
   * @param firstMove
  ...
  public double score(int i) {
    ...
    throw new IllegalArgumentException("Looking for posting list " + i
