 ...
 * documents for each query term and modifying these scores with the appropriate
 * modifiers. Then, a series of document score modifiers are applied if
 * necessary.
 * <p>
 * <b>Properties</b>
 * <ul>
 * <li><tt>matching.retrieved_set_size</tt> - The maximum number of documents in
 * the final retrieved set. The default value is 1000, however, setting the
 * property to 0 will return all matched documents.</li>
 * <li><tt>ignore.low.idf.terms</tt> - A property that enables to ignore the
 * terms with a low IDF.</li>
 * <li><tt>match.empty.query</tt> - whether an empty query should return all
 * documents. Defaults to false.</li>
 * </ul>
 * 
 * @since 3.0
...
public abstract class BaseMatching implements Matching {
  ...
  /**
   * The maximum number of documents in the final retrieved set. It corresponds
   * to the property <tt>matching.retrieved_set_size</tt>. The default value is
   * 1000, however, setting the property to 0 will return all matched documents.
   */
   ...
   * specifically, it is the ordering of documents in the document index.
   */
  ...
  public BaseMatching(Index _index) {
    ...
    try {
      for (String modifierName : defaultDSMS.split("\\s*,\\s*")) {
        ...
        if (modifierName.indexOf('.') == -1) modifierName = dsmNamespace
            + modifierName;
        documentModifiers.add((DocumentScoreModifier) Class.forName(
            modifierName).newInstance());
      }
    } catch (Exception e) {
      logger
          .error(
              "Exception while initialising default modifiers. Please check the name of the modifiers in the configuration file.",
              e);
    }
  }
  ...
  protected void initialisePostings(MatchingQueryTerms queryTerms) {
    ...
    for (String queryTerm : queryTermStrings) {
      ...
      if (t != null) {
        ...
        WeightingModel[] termWeightingModels = queryTerms
            .getTermWeightingModels(queryTerm);
        if (termWeightingModels.length == 0) {
          logger.warn("No weighting models for term " + queryTerm
              + ", skipping scoring");
          continue;
        }
        queryTermsToMatchList.add(new MapEntry<String,LexiconEntry>(queryTerm,
            t));
      } else logger.debug("Term Not Found: " + queryTerm);
    }
    ...
    for (int i = 0; i < queryLength; i++) {
      ...
      // if none were provided with the query we seek the entry statistics query
      // term in the lexicon
      if (entryStats == null) {
        entryStats = lexiconEntry;
        // save them as they may be useful for query expansion. HOWEVER ONLY IF
        // we didnt
      ...
      for (int j = 0; j < numWM; j++) {
        wm[i][j] = (WeightingModel) queryTerms
            .getTermWeightingModels(queryTerm)[j].clone();
  ...
  protected void initialise(MatchingQueryTerms queryTerms) {
    ...
    RETRIEVED_SET_SIZE = Integer.parseInt(ApplicationSetup.getProperty(
        "matching.retrieved_set_size", "1000"));
    IGNORE_LOW_IDF_TERMS = Boolean.parseBoolean(ApplicationSetup.getProperty(
        "ignore.low.idf.terms", "false"));
    MATCH_EMPTY_QUERY = Boolean.parseBoolean(ApplicationSetup.getProperty(
  ...
  protected void finalise(MatchingQueryTerms queryTerms) {
    ...
    logger
        .debug("Number of docs with +ve score: " + numberOfRetrievedDocuments);
    ...
    HeapSort.descendingHeapSort(resultSet.getScores(), resultSet.getDocids(),
        resultSet.getOccurrences(), set_size);
    // long sortingEnd = System.currentTimeMillis();
    
    /*
     * we apply the query dependent document score modifiers first and then we
     * apply the application dependent ones. This is to ensure that the
     * BooleanFallback modifier is applied last. If there are more than one
     * application dependent dsms, then it's up to the application, ie YOU! to
     * ensure that the BooleanFallback is applied last.
     */
    
    /*
     * dsms each require resorts of the result list. This is expensive, so
     * should be avoided if possible. Sorting is only done if the dsm actually
     * altered any scores
     */
    
    /*
     * query dependent modification of scores of documents for a query, defined
     * by this query
     */
    ...
    for (int t = NumberOfQueryDSMs - 1; t >= 0; t--) {
      if (dsms[t].modifyScores(index, queryTerms, resultSet)) HeapSort
          .descendingHeapSort(resultSet.getScores(), resultSet.getDocids(),
              resultSet.getOccurrences(), resultSet.getResultSize());
    }
    
    /*
     * application dependent modification of scores of documents for a query,
     * based on a static set by the client code sorting the result set after
     * applying each DSM
     */
    for (int t = 0; t < numOfDocModifiers; t++) {
      if (documentModifiers.get(t).modifyScores(index, queryTerms, resultSet)) HeapSort
          .descendingHeapSort(resultSet.getScores(), resultSet.getDocids(),
              resultSet.getOccurrences(), resultSet.getResultSize());
    }
  ...
  /** {@inheritDoc} */
  @Override
  public abstract ResultSet match(String queryNumber,
